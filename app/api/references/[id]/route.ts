...
export async function PUT(req: Request, { params }: Ctx) {
  try {
    ...
    `, [body.citation_key, body.title, body.authors, body.year, body.journal, body.doi, body.url, body.notes, body.read_status, id, userId]);
    ...
    const ref = await row<Reference>('SELECT * FROM refs WHERE id = ? AND user_id = ?', [id, userId]);
...
export async function DELETE(_req: Request, { params }: Ctx) {
  try {
    const userId = await requireUserId();
    const id = parseInt(params.id, 10);
    await run('DELETE FROM refs WHERE id = ? AND user_id = ?', [id, userId]);
